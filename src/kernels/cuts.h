...
template <typename T>
KERNELSPEC void kernel_cut_pt(FIn<T, 2> p, FIn<T, 2> min_max, FOut<T, 0> w) {
    FVal<T> ret(1.);
...
template <typename T>
KERNELSPEC void kernel_cut_eta(FIn<T, 2> p, FIn<T, 2> min_max, FOut<T, 0> w) {
    FVal<T> ret(1.);
    for (std::size_t i = 0; i < min_max.size(); ++i) {
...
template <typename T>
KERNELSPEC void
kernel_cut_dr(FIn<T, 2> p, IIn<T, 2> indices, FIn<T, 2> min_max, FOut<T, 0> w) {
    FVal<T> ret(1.);
    for (std::size_t i = 0; i < indices.size(); ++i) {
        auto indices_i = indices[i];
        auto p1 = p[single_index(indices_i[0]) + 2],
             p2 = p[single_index(indices_i[1]) + 2];
...
template <typename T>
KERNELSPEC void
kernel_cut_m_inv(FIn<T, 2> p, IIn<T, 2> indices, FIn<T, 2> min_max, FOut<T, 0> w) {
    FVal<T> ret(1.);
    for (std::size_t i = 0; i < indices.size(); ++i) {
...
template <typename T>
KERNELSPEC void kernel_cut_sqrt_s(FIn<T, 0> sqrt_s, FIn<T, 1> min_max, FOut<T, 0> w) {
