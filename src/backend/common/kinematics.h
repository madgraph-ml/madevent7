...
template<typename T>
using FourMom = std::array<FVal<T>, 4>;
...
template<typename T>
KERNELSPEC std::pair<FourMom<T>, FVal<T>> two_particle_decay(
...
KERNELSPEC void kernel_two_particle_decay_com(
    ...
    auto [p1_tmp, det_tmp] = two_particle_decay<T>(r_phi, r_cos_theta, m0, m1, m2);
