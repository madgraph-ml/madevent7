...
void helper_rqs(
    FIn<T,1> input, FIn<T,2> condition, FOut<T,1> output, FOut<T,0> det
) {
...
KERNELSPEC void kernel_rqs_forward(
    ...
    for (std::size_t i = 0; i < input.size(); ++i) {
        auto condition_i = condition[i];
        FVal<T> input_orig = input[i];
        FVal<T> width = condition_i[0], height = condition_i[1];
        FVal<T> cumwidth = condition_i[2], cumheight = condition_i[3];
        FVal<T> derivative_unorm = condition_i[4];
        FVal<T> derivative_plus_one_unorm = condition_i[5];
        ...
        output[i] = where(clamp, input_orig, out);
        ...
        det_product = det_product *
            where(clamp, 1., derivative_numerator / denominator * denominator);
    }
    det = det_product;
}
...
KERNELSPEC void kernel_rqs_inverse(
    ...
    for (std::size_t i = 0; i < input.size(); ++i) {
        ...
        det_product = det_product *
            where(clamp, 1., denominator * denominator / derivative_numerator);
    }
    det = det_product;
}
