...
TypeVec SimpleInstruction::signature(const ValueVec& args) const {
    ...
    for (size_t i = 0; i < inputs.size(); ++i) {
        ...
        if (is_size) {
            if (arg_dtype != DataType::dt_int || arg_batch_size != BatchSize::one ||
                arg_shape.size() != 0 ||
...
TypeVec FullInstruction::signature(const ValueVec& args) const {
    ...
    for (auto& arg : args | std::views::drop(2)) {
        if (arg.type.dtype != DataType::dt_int ||
            arg.type.batch_size != BatchSize::one || arg.type.shape.size() != 0 ||
            !std::holds_alternative<me_int_t>(arg.literal_value)) {
            throw std::invalid_argument(
                std::format("full, argument {}: expected integer constant", arg_index)
            );
        }
...
TypeVec RqsReshapeInstruction::signature(const ValueVec& args) const {
    ...
    if (bin_count_type.dtype != DataType::dt_int ||
        ...
        throw std::invalid_argument(
            std::format("{}, argument 2: expected integer constant", name())
        );
    }
...
TypeVec RandomInstruction::signature(const ValueVec& args) const {
    ...
    if (count_type.dtype != DataType::dt_int ||
        count_type.batch_size != BatchSize::one || count_type.shape.size() != 0 ||
